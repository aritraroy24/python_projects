...
# logging config
logging.basicConfig(level=logging.INFO, filename='data.log',)
...
def auto_tweet():
    ...
    for text in text_list:
        try:
            ...
            logger.info(f"Tweet done at : {str(datetime.datetime.now())}\n\n\n")

        except Exception as e:
            print(f"{str(datetime.datetime.now())}  =====  {e}")
            logger.info(f"Tweet can't be done at : {str(datetime.datetime.now())} due to {e} error\n\n\n")
